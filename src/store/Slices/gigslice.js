...
import { DeleteServiceApi, GetAllServiceApi, GetCurrentUserServicesApi } from "../../Api_Requests/Api_Requests";

export const fetchGigs = createAsyncThunk("gigs/fetchGigs", async (_, { rejectWithValue }) => {
    try {
...
export const deleteGig=createAsyncThunk("gigs/deleteGig",async (id,{rejectWithValue})=>{
  ...
  export const fetchCurrentUserGigs = createAsyncThunk("gigs/fetchCurrentUserGigs", async (_, { rejectWithValue }) => {
    try {
    ...
    extraReducers: (builder) => {
        builder
          ...
          .addCase(fetchCurrentUserGigs.rejected, (state, action) => {
            ...
            state.loading = false;
            state.deleteGigError = null;
            state.personalGigs=state.personalGigs.filter((gig)=>gig._id!==action.payload)
          })
      },

})
export const {setId,addGig} = gigslice.actions
